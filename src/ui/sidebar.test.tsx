...
async function setup(backend?: SpawnBackend, agentsOnly = false, initial?: AgentOptions) {
  ...
  const { spaces, dispose: disposeSpaces } = scopedSpaceSet(
    workspaceEnv(t.renderer, { shell: SHELL, backend }),
    paneHost,
  );
...
test("agents-only filtering removes plain panes and empty parents", async () => {
  ...
  try {
    ...
    expect(targets.map((target) => target.kind)).toEqual(["space", "window", "agent"]);
...
test("a long OSC title is truncated so the foreground command stays visible", async () => {
  ...
  try {
    ...
    await waitFor(
      () => s.t.captureCharFrame().includes("sleep · dan@"),
      "the titled command row to render",
    );
    const frame = s.t.captureCharFrame();
...
test("the seam shows a thumb only while the tree overflows", async () => {
  ...
  const paneHost = new BoxRenderable(t.renderer, { id: "pane-host", flexGrow: 1 });
  const { spaces, dispose: disposeSpaces } = scopedSpaceSet(
    workspaceEnv(t.renderer, { shell: SHELL }),
    paneHost,
  );
  const space = Effect.runSync(spaces.create("proj", process.cwd()));
  const win = Effect.runSync(space.newWindow());
  ...
  try {
    ...
    for (let i = 0; i < 18; i++)
      await Effect.runPromise(win.killAgent(win.agents[win.agents.length - 1]!));
    spaces.onChange?.();
