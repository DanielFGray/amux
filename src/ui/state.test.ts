...
function scopedRunner() {
  ...
  const run = Effect.runSync(Scope.extend(Effect.gen(function* () {
...
test("the UI poll keeps one fixed cadence and does not slow down when idle", async () => {
  ...
  let polls = 0
  fibers.run("ui-poll", scheduledPoll(POLL_MS, () => { polls++ }))
...
test("closing the app fiber scope stops the UI poll", async () => {
  const fibers = scopedRunner()
  let polls = 0
  fibers.run("ui-poll", scheduledPoll(10, () => { polls++ }))
...
test("output does not advance the polled tick, so a busy pane cannot storm the tree", () => {
  ...
  const spaces = { spaces: [], allAgents: [], active: null, activeWindow: null } as unknown as SpaceSet
...
test("a replacement hint delay cancels the previous callback", async () => {
  ...
  scheduleHintVisibility(fibers.run, 40, () => true, () => shown.push("first"))
  await Bun.sleep(5)
  scheduleHintVisibility(fibers.run, 10, () => true, () => shown.push("second"))
  await Bun.sleep(50)
...
test("closing the app fiber scope prevents a pending hint callback", async () => {
  const fibers = scopedRunner()
  let callbacks = 0
  scheduleHintVisibility(fibers.run, 20, () => true, () => callbacks++)
...
test("model projection starts in stream order and stops with the scope", async () => {
  ...
  fibers.run("workspace-models", runModelProjections(models, async (model) => {
