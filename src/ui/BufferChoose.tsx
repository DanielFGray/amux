  ...
  return (
    ...
    >
      ...
      <Show
        ...
        fallback={
          <text style={{ fg: theme.overlay1, height: 1 }}>no buffers — copy something first</text>
        }
      >
        <scrollbox
          ref={(value) => {
          ...
          <For each={props.view.buffers}>
            {(buffer, index) => (
              ...
              >
                ...
                <text style={{ fg: theme.subtext0, width: 8, flexShrink: 0 }}>
                  {buffer.bytes} b
                </text>
                <text style={{ fg: theme.text, flexGrow: 1 }}>{buffer.preview || "(empty)"}</text>
              </box>
            )}
          </For>
        </scrollbox>
