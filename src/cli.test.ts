...
test("commands use the pane session unless an explicit session is supplied", () => {
  ...
  try {
    expect(resolveCommandSession("notify", [], { title: "t", body: "b" })).toBe("pane-session");
    expect(
      resolveCommandSession("notify", [], { title: "t", body: "b", session: "override" }),
    ).toBe("override");
    expect(resolveCommandSession("pane.next", [], {})).toBe("pane-session");
  } finally {
...
test("ordinary commands retain the default session outside a managed pane", () => {
  ...
  try {
    expect(resolveCommandSession("pane.next", [], {})).toBe("default");
    expect(resolveCommandSession("notify", [], { title: "t", body: "b" })).toBeNull();
  } finally {
    if (previous === undefined) delete process.env.AMUX_DAEMON_SESSION;
    else process.env.AMUX_DAEMON_SESSION = previous;
  }
