...
const layout = (root: LayoutNode, focus?: string): Layout => ({ version: LAYOUT_VERSION, root, focus })
...
test("borders stay inside pane rectangles and nested axes use their parent's exact allocation", () => {
  const rects = computeRects(layout(split("row", [
    pane("left"),
  ...
  expect(computeRects(layout(pane("only")), { cols: 20, rows: 11 }).get("only"))
    .toEqual({ x: 0, y: 0, width: 20, height: 11 })
})
...
test("pure rectangles are a fixed point of OpenTUI flex layout", async () => {
  ...
  window.applyLayout(makeLayout(split("row", [
