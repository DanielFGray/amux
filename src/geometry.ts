...
export function paneInDirection(
  ...
  for (const [id, rect] of rects) {
    ...
    const overlap = Math.min(crossEnd(from), crossEnd(rect)) - Math.max(crossStart(from), crossStart(rect))
...
export function resizeDivider(
  ...
  const replacement: LayoutSplit = {
    ...
    children: split.children.map((child, at) => ({ ...child, weight: Math.max(0.0001, sizes[at]!) })),
  }
...
function replaceAt(root: LayoutNode | null, path: LayoutPath, replacement: LayoutNode): LayoutNode | null {
  if (path.length === 0) return replacement
  if (root?.type !== "split") return root
  const [index, ...rest] = path
  return {
    ...root,
    children: root.children.map((child, at) => at === index ? replaceAt(child, rest, replacement)! : child),
  }
}
...
function pathHasNeighbour(
  ...
  for (const index of path) {
    ...
    if (node.direction === axis && (side < 0 ? index > 0 : index < node.children.length - 1)) return true
    node = node.children[index] ?? null
  }
  return false
}
...
function containsPane(node: LayoutNode | undefined, paneId: string): boolean {
  ...
  return node.type === "pane" ? node.id === paneId : node.children.some((child) => containsPane(child, paneId))
}
