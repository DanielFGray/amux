...
// Panes.
const PaneSplit = define("pane.split", { axis: Axis }, {
...
const PaneSwap = define("pane.swap", { to: Schema.Literal("previous", "next") }, {
  desc: "swap the focused pane with its neighbour",
  group: "panes",
  capability: "agent",
})
const PaneClose = define("pane.close", {}, {
  desc: "close the focused pane, leaving its agent running",
  group: "panes",
  capability: "agent",
})
const PaneBreak = define("pane.break", {}, {
  desc: "break the focused pane into its own window",
  group: "panes",
  capability: "agent",
})
const PaneSendKeys = define("pane.send-keys", { keys: Schema.String }, {
...
const PaneCapture = define("pane.capture", {}, {
  desc: "capture the focused pane into a popup",
  group: "panes",
  capability: "local",
})
const PaneCopyMode = define("pane.copy-mode", {}, {
...
const WindowSelect = define("window.select", { ...Space, number: Schema.Int }, {
  desc: "select a window by its number",
  group: "windows",
  capability: "agent",
})
const WindowRename = define("window.rename", { ...Window, name: Schema.String }, {
...
const WindowNextLayout = define("window.next-layout", {}, {
  desc: "cycle through the preset layouts",
  group: "windows",
  capability: "agent",
})
const WindowSelectLayout = define("window.select-layout", { preset: Schema.Literal(...LAYOUT_PRESETS) }, {
  desc: "arrange panes in a preset layout",
  group: "windows",
  capability: "agent",
})
const WindowSynchronize = define("window.synchronize-panes", {}, {
...
const SpaceRename = define("space.rename", { ...Space, name: Schema.String }, {
  desc: "rename a space",
  group: "spaces",
  capability: "agent",
})
...
const SpaceNext = define("space.next", {}, { desc: "next space", group: "spaces", capability: "agent" })
const SpacePrevious = define("space.previous", {}, {
  desc: "previous space",
  group: "spaces",
  capability: "agent",
})
...
const AppSettings = define("app.settings", {}, { desc: "settings", group: "global", capability: "local" })
const AppSendPrefix = define("app.send-prefix", {}, {
 ...
 */
export const command = <T extends CommandTag>(tag: T, ...args: {} extends ArgsOf<T> ? [args?: ArgsOf<T>] : [args: ArgsOf<T>]): Command =>
  ({ _tag: tag, ...args[0] }) as Command
...
export const makeCommands = (handlers: CommandHandlers): Commands => ({
  ...
  list: (floor = "local") =>
    COMMAND_DEFS.filter((def) => atLeast(def.capability, floor)).map((def) => COMMAND_META[def.tag]),
})
