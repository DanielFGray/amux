...
const untilExit = (frames: Stream.Stream<AttachFrame>) =>
  Stream.runCollect(
    Stream.takeUntil(frames, (frame) => frame._tag === "exit"),
    ...
    yield* Effect.gen(function* () {
      ...
      Effect.provide(
        SessionSupervisor.Live.pipe(
          Layer.provide(Layer.succeed(AgentLog, log)),
        ),
      ),
      Effect.provide(AttachHub.Default),
    );
 ...
 */
testEffect(
  "a supervised session publishes its foreground process, and its changes",
  () =>
    Effect.gen(function* () {
      ...
      yield* Deferred.await(atPrompt).pipe(
        ...
        Effect.orElseSucceed(() => {
          throw new Error(
            "no foreground frame with a shell at a prompt arrived",
          );
        }),
      );
 ...
 */
testEffect(
  "scope teardown kills a session left running, without an explicit kill",
  () =>
    Effect.gen(function* () {
      const supervisor = yield* SessionSupervisor;
      yield* supervisor.spawn({
...
testEffect(
  "exit cleanup releases the session and replay terminal for reuse",
  () =>
    Effect.gen(function* () {
      const hub = yield* AttachHub;
      const subscription = yield* hub.subscribe("client");
...
testEffect(
  "concurrent supervisor kills publish one exit and permit same-id reuse",
  () =>
    Effect.gen(function* () {
      const hub = yield* AttachHub;
      const subscription = yield* hub.subscribe("client");
      ...
      const collector = yield* Effect.fork(
        Stream.runForEach(subscription.frames, (frame) =>
          Effect.sync(() => received.push(frame)).pipe(
            Effect.zipRight(
              frame._tag === "exit"
                ? Deferred.succeed(exitSeen, void 0)
                : Effect.void,
            ),
          ),
        ),
...
testEffect(
  "a native agent worker is listed and killed through the supervisor",
  () =>
    Effect.gen(function* () {
      const hub = yield* AttachHub;
      const subscription = yield* hub.subscribe("client");
