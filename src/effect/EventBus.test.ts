...
testEffect(
  "EventBus broadcasts events and releases subscriptions with scope",
  () =>
    Effect.gen(function* () {
      ...
      expect([...events.one]).toEqual([
        {
          sequence: 1,
          event: { _tag: "agent.state", session: "agent-1", state: "blocked" },
        },
      ]);
...
testEffect(
  "EventBus uses sliding delivery rather than blocking publishers",
  () =>
    Effect.gen(function* () {
      const bus = yield* EventBus;
