  ...
  Effect.gen(function* () {
    ...
    const dispatchFrame = (socket: Bun.Socket<ClientState>, frame: AttachFrame) => {
      ...
      const current = state.run(
        `frame:${state.nextFiber++}`,
        Effect.forEach(dependencies, (fiber) => (fiber ? Fiber.await(fiber) : Effect.void), {
          discard: true,
        }).pipe(
    ...
    const processData = (socket: Bun.Socket<ClientState>, data: Buffer) => {
      ...
      const current = state.run(
        `wire:${state.nextFiber++}`,
        (previous ? Fiber.await(previous) : Effect.void).pipe(
          Effect.andThen(
            Effect.gen(function* () {
    ...
    const closeClient = (socket: Bun.Socket<ClientState>) =>
      Effect.uninterruptible(
        Effect.gen(function* () {
          ...
          if (client)
            yield* (options.onDetach?.(client, connection) ?? Effect.void).pipe(Effect.exit);
        }),
      );
