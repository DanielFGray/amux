...
const open = (id: string, e: NodeJS.ProcessEnv, options: SessionDaemonOptions = {}) => run(SessionDaemon.open(id, options), e)
const paths = (id: string, e: NodeJS.ProcessEnv) => run(sessionPaths(id), e)
const context = { size: { cols: 80, rows: 24 }, shell: ["sh"], cwd: "/tmp" }
const saveEffect = (save: (state: any, signal: AbortSignal) => Promise<void>) =>
  (state: any) => Effect.tryPromise({ try: (signal) => save(state, signal), catch: (error) => error })
...
async function expectProcessGone(pid: number): Promise<void> {
  const deadline = Date.now() + 2_000
  while (Date.now() < deadline) {
    try { await readFile(`/proc/${pid}/stat`) } catch { return }
    await Bun.sleep(10)
  }
...
test("the daemon-owned workspace survives closing and reopening", async () => {
  ...
  await first.start()
  await first.runWorkspaceCommand(command("space.rename", { name: "proj" }), first.workspace.revision, context)
  await first.runWorkspaceCommand(command("window.rename", { name: "build" }), first.workspace.revision, context)
  await first.close()
...
test("concurrent status reads an empty workspace without racing default creation", async () => {
  ...
  await daemon.runWorkspaceCommand(command("space.close", { space }), daemon.workspace.revision, context)
...
test("stopping waits for an in-flight workspace mutation before removing metadata", async () => {
  ...
  const save = daemon.runWorkspaceCommand(command("space.rename", { name: "p" }), daemon.workspace.revision, context)
...
test("RPC rejects aggregate command bodies before decoding their payload", async () => {
  ...
  await daemon.start()
  const response = await run(daemonRequest("bounded-rpc", {
    command: "set-buffer",
...
test("a fast prepared exit cannot deadlock failed-write compensation", async () => {
  ...
  const daemon = await open("fast-compensation", e, {
    saveState: saveEffect(async (state: any) => {
      const agents = state.spaces.flatMap((space: any) => space.windows.flatMap((window: any) => window.agents))
  ...
  await expect(Promise.race([
    ...
    Bun.sleep(1_000).then(() => { throw new Error("compensation deadlocked") }),
...
test("a prepared session is absent from status and subscribers until its model is durable", async () => {
  ...
  const commandRun = daemon.runWorkspaceCommand(command("pane.split", { axis: "row" }), daemon.workspace.revision, {
    ...context,
  ...
  expect(await daemon.liveAgents()).toEqual(beforeLive)
  expect(await Promise.race([model.then(() => "published"), Bun.sleep(30).then(() => "private")])).toBe("private")
  expect(await Promise.race([terminal.then(() => "published"), Bun.sleep(30).then(() => "private")])).toBe("private")
...
test("a one-shot reversible write failure does not poison the next command", async () => {
  ...
  await expect(daemon.runWorkspaceCommand(command("pane.split", { axis: "row" }), revision, context))
    .rejects.toThrow("one-shot candidate failure")
  expect((await daemon.handle({ command: "status" })).ok).toBe(true)
  const recovered = await daemon.runWorkspaceCommand(command("pane.split", { axis: "row" }), revision, context)
...
test("a rejected candidate never reaches current or backup state", async () => {
  ...
  await daemon.start()
  await expect(daemon.runWorkspaceCommand(
  ...
  const agents = reopened.state.spaces.flatMap((space) => space.windows.flatMap((window) => window.agents))
...
test("attachment metadata cannot overwrite a newer workspace generation", async () => {
  ...
  const detaching = new Promise<void>((resolve) => { detachStarted = resolve })
  ...
  await started
  let renamed = false
  const rename = daemon.runWorkspaceCommand(
...
test("a destructive commit retries its single durable write after process completion", async () => {
  ...
  await daemon.runWorkspaceCommand(command("agent.kill", { agent }), daemon.workspace.revision, context)
...
test("stop interrupts and joins a never-settling destructive persistence operation", async () => {
  ...
  const daemon = await open("kill-save-cancel", e, {
    saveState: (state) => {
      if (armed && state.spaces.length === 0) {
        saving()
        return Effect.never.pipe(Effect.ensuring(Effect.sync(() => { cancelled = true })))
      }
  ...
  await daemon.spawnAgent({ id: "held-destructive", cmd: ["sh", "-c", `printf '%s' $$ > ${marker}; sleep 30`], cols: 80, rows: 24 })
  const heldPid = await waitForPid(marker)
  armed = true
  const agent = daemon.workspace.spaces[0]!.windows[0]!.agents[0]!.id
  const mutation = daemon.runWorkspaceCommand(command("agent.kill", { agent }), daemon.workspace.revision, context)
  void mutation.catch(() => {})
  await saveStarted
  const started = Date.now()
  await Promise.race([
    daemon.stop(),
    Bun.sleep(1_500).then(() => { throw new Error("stop did not interrupt destructive persistence") }),
  ])
...
test("a transient natural-exit write failure retries before making the exit visible", async () => {
  ...
  const daemon = await open("exit-retry", e, {
    saveState: saveEffect(async (state: any) => {
      const exited = state.spaces.flatMap((space: any) => space.windows)
        .flatMap((window: any) => window.agents).some((agent: any) => agent.exited)
      if (exited && !failed) {
        failed = true
  ...
  await daemon.start()
  await daemon.runWorkspaceCommand(command("pane.split", { axis: "row" }), daemon.workspace.revision, {
    ...context,
    shell: ["sh", "-c", "exit 7"],
  })
  const deadline = Date.now() + 2_000
  while (!daemon.workspace.spaces[0]!.windows[0]!.agents.some((agent) => agent.exited) && Date.now() < deadline) {
    await Bun.sleep(10)
  }
  expect(failed).toBe(true)
  expect(daemon.workspace.spaces[0]!.windows[0]!.agents.some((agent) => agent.exited && agent.exitCode === 7)).toBe(true)
  expect((await daemon.handle({ command: "status" })).ok).toBe(true)
  await daemon.stop()
})
...
test("permanent natural-exit persistence failure surfaces unhealthy status until recovery", async () => {
  ...
  let attached = false
  const connecting = AttachClient.connect({ path: p.attach, client: "blocked-metadata" })
    .then((client) => { attached = true; return client })
  await Bun.sleep(30)
  ...
  while (!(await daemon.handle({ command: "status" })).ok && Date.now() < recovered) await Bun.sleep(10)
  expect((await daemon.handle({ command: "status" })).ok).toBe(true)
  client.close()
  await daemon.stop()
})
...
test("close interrupts and joins a never-settling natural-exit persistence operation", async () => {
  ...
  const daemon = await open("exit-save-cancel", e, {
    saveState: (state) => {
      const exited = state.spaces.flatMap((space) => space.windows)
        .flatMap((window) => window.agents).some((agent) => agent.exited)
      if (armed && exited) {
        saving()
        return Effect.never.pipe(Effect.ensuring(Effect.sync(() => { cancelled = true })))
      }
  ...
  await daemon.spawnAgent({ id: "held-natural", cmd: ["sh", "-c", `printf '%s' $$ > ${marker}; sleep 30`], cols: 80, rows: 24 })
  const heldPid = await waitForPid(marker)
  armed = true
  await daemon.runWorkspaceCommand(command("pane.split", { axis: "row" }), daemon.workspace.revision, {
    ...context,
  ...
  await Promise.race([
    daemon.close(),
    Bun.sleep(1_500).then(() => { throw new Error("close did not interrupt natural-exit persistence") }),
  ])
  expect(Date.now() - started).toBeLessThan(1_500)
  expect(cancelled).toBe(true)
  expect(await daemon.liveAgents()).toEqual([])
...
test("a failed destructive action leaves durable state untouched", async () => {
  ...
  ;(daemon as any).killAgent = async () => { throw new Error("injected kill failure") }
  await expect(
    daemon.runWorkspaceCommand(command("agent.kill", { agent }), before.revision, context),
  ).rejects.toThrow("injected kill failure")
  expect(daemon.workspace).toEqual(before)
  expect((await run(loadSession("kill-transaction"), e))?.spaces[0]?.windows[0]?.agents[0]?.id).toBe(agent)
...
test("restore spawn failures are persisted before the daemon accepts clients", async () => {
  const e = await env()
  const layout = JSON.stringify({ version: 1, root: { type: "pane", id: "pane-restore", agent: "agent-restore", weight: 1 }, focus: "pane-restore" })
  await run(saveSession({
    version: 1, id: "restore-failure", createdAt: 1, updatedAt: 1, attached: false, activeSpace: "space-restore",
    spaces: [{ id: "space-restore", name: "restore", dir: "/tmp", activeWindow: 1, windows: [{
      number: 1, name: null, layout,
      agents: [{ id: "agent-restore", name: "bad", cmd: ["bad"], cols: 80, rows: 24, exited: false, exitCode: null }],
    }] }],
  }), e)
