...
import { parseSessionState, SESSION_VERSION, type PersistedAgent, type SessionState } from "./session.ts"
import {
  activateSpaceState,
  claimWindowNumber,
  closeWindowState,
...
export function workspaceFromSession(session: SessionState): WorkspaceSnapshot {
  ...
  return {
    revision: 0,
    state: { activeSpace: session.spaces.some((space) => space.id === session.activeSpace)
    ...
    spaces: session.spaces.map((saved) => {
      const windows = saved.windows.map((window): WorkspaceWindow => {
        const live = new Set(window.agents.filter((agent) => !agent.exited).map((agent) => agent.id))
        let layout: Layout | null = null
        try {
          layout = window.layout ? prune(decodeLayout(window.layout), (agent) => live.has(agent)) : null
        } catch {}
        ...
        if (layout.root && !layout.focus) layout = makeLayout(layout.root, layoutPanes(layout.root)[0]?.id)
        const state = windowState()
        state.focus = layout.focus ?? null
        return { number: window.number, name: window.name, agents: structuredClone(window.agents), layout, state }
      })
...
export function parseWorkspace(value: unknown): WorkspaceSnapshot {
  if (!record(value) || !Number.isSafeInteger(value.revision) || (value.revision as number) < 0 ||
    !Array.isArray(value.spaces) || !record(value.state) ||
  ...
  parseSessionState(workspaceSession(raw, {
    version: SESSION_VERSION,
    id: "workspace",
    createdAt: 0,
    updatedAt: 0,
  ...
  for (const space of raw.spaces) {
    if (!record(space.state) || !(space.state.activeWindow === null || positiveInt(space.state.activeWindow)) ||
      !(space.state.lastWindow === null || positiveInt(space.state.lastWindow)) || !positiveInt(space.state.nextWindow)) {
      throw new Error("workspace has invalid space state")
    }
    const numbers = new Set(space.windows.map((window) => window.number))
    if ((space.state.activeWindow !== null && !numbers.has(space.state.activeWindow)) ||
    ...
    for (const window of space.windows) {
      if (!record(window.state) || !(window.state.focus === null || typeof window.state.focus === "string") ||
      ...
      if (window.state.focus !== (window.layout.focus ?? null) ||
      ...
      if (window.state.zoom !== null) {
        ...
        const agents = new Set(window.agents.filter((agent) => !agent.exited).map((agent) => agent.id))
...
export function parseWorkspaceCommandContext(value: unknown, workspace?: WorkspaceSnapshot): WorkspaceCommandContext {
  ...
  if (workspace) {
    const agents = new Set(workspace.spaces.flatMap((space) => space.windows.flatMap((window) => window.agents.map((agent) => agent.id))))
    if (blocked.some((id: string) => !agents.has(id))) throw new Error("blocked agent does not exist")
  }
...
export function applyWorkspaceCommand(
  ...
  const agentIds = new Set(next.spaces.flatMap((space) => space.windows.flatMap((window) => window.agents.map((agent) => agent.id))))
  const paneIds = new Set(next.spaces.flatMap((space) => space.windows.flatMap((window) => layoutPanes(window.layout.root).map((pane) => pane.id))))
  ...
  const addWindow = (target: WorkspaceSpace): WorkspaceWindow => {
    ...
    target.state = selectWindowState(target.state, target.windows.map((item) => item.number), number)
  ...
  switch (command._tag) {
    ...
    case "pane.focus": {
      ...
      setFocus(target.window, paneInDirection(target.window.layout, context.size, target.window.state.focus, command.direction) ?? undefined)
      break
    }
    case "pane.select": {
    ...
    case "pane.resize": {
      ...
      const resized = resizePane(target.window.layout, context.size, target.window.state.focus, command.direction)
    ...
    case "pane.resize-divider": {
      ...
      const resized = resizeDivider(target.layout, context.size, command.path, command.index, command.delta)
    ...
    case "pane.zoom": {
      ...
      target.state.zoom = target.state.zoom ? null : { pane: target.state.focus, from: target.layout }
      break
    }
    case "pane.swap": {
      const target = activeWindow()?.window
      ...
      if (at !== -1 && panes.length > 1) {
        target.layout = swapLayout(target.layout, at, (at + (command.to === "next" ? 1 : -1) + panes.length) % panes.length)
    ...
    case "pane.send-keys": {
      const target = activeWindow()?.window
      const focused = layoutPanes(target?.layout.root ?? null).find((pane) => pane.id === target?.state.focus)
      ...
      } else if (focused) actions.push({ _tag: "input", agent: focused.agent, data: context.input ?? command.keys })
      break
    }
    case "window.new": {
      const target = space()
    ...
    case "window.previous": {
      ...
      target.state = selectWindowState(target.state, target.windows.map((item) => item.number), target.windows[(at + step + target.windows.length) % target.windows.length]!.number)
      break
    }
    case "window.last": {
      const target = space()
      if (target && target.state.lastWindow !== null) {
        target.state = selectWindowState(target.state, target.windows.map((item) => item.number), target.state.lastWindow)
      }
      break
    }
    case "window.select": {
      const target = space()
      if (target) target.state = selectWindowState(target.state, target.windows.map((item) => item.number), command.number)
      break
    }
    case "window.rename": {
      const target = window()
    ...
    case "window.select-layout": {
      const target = activeWindow()?.window
      if (!target) break
      const preset = command._tag === "window.next-layout" ? nextPreset(target.state.preset) : command.preset
      target.layout = presetLayout(layoutPanes(target.layout.root), preset, target.state.focus ?? undefined)
      target.state.zoom = null
    ...
    case "agent.reveal": {
      ...
      next.state = activateSpaceState(next.state, next.spaces.map((space) => space.id), target.space.id)
      target.space.state = selectWindowState(target.space.state, target.space.windows.map((window) => window.number), target.window.number)
      let pane = layoutPanes(target.window.layout.root).find((candidate) => candidate.agent === target.agent.id)
      if (!pane) {
        ...
        target.window.layout = slots.length
          ? splitLayout(target.window.layout, Math.max(0, slots.findIndex((candidate) => candidate.id === target.window.state.focus)), "row", ref)
          : makeLayout({ type: "pane", ...ref, weight: 1 }, ref.id)
    ...
    case "agent.next-blocked": {
      ...
      const currentAgent = next.spaces.flatMap((item) => item.windows).flatMap((item) => layoutPanes(item.layout.root)).find((pane) => pane.id === focused)?.agent
      ...
      if (target) {
        next.state = activateSpaceState(next.state, next.spaces.map((item) => item.id), target.space.id)
        target.space.state = selectWindowState(target.space.state, target.space.windows.map((item) => item.number), target.window.number)
    ...
    case "space.select": {
      next.state = activateSpaceState(next.state, next.spaces.map((space) => space.id), command.space)
      break
    }
    case "space.rename": {
      const target = space()
    ...
    case "space.previous": {
      ...
      next.state = activateSpaceState(next.state, next.spaces.map((item) => item.id), next.spaces[(at + step + next.spaces.length) % next.spaces.length]!.id)
      break
    }
  }
  ...
  return { snapshot: changed ? { ...next, revision: current.revision + 1 } : current, actions, changed }
}

const WORKSPACE_COMMANDS = new Set<Command["_tag"]>([
  "pane.split", "pane.next", "pane.last", "pane.focus", "pane.select", "pane.resize", "pane.resize-divider",
  "pane.zoom", "pane.swap", "pane.close", "pane.break", "pane.send-keys", "window.new", "window.next", "window.previous",
  "window.last", "window.select", "window.rename", "window.close", "window.next-layout",
  "window.select-layout", "window.synchronize-panes", "agent.kill", "agent.reveal", "agent.next-blocked", "space.new",
  "space.select", "space.rename", "space.close", "space.next", "space.previous",
])

export const isWorkspaceCommand = (command: Command): boolean => WORKSPACE_COMMANDS.has(command._tag)

/** Natural PTY exit is a daemon-side model mutation too. */
export function markAgentExited(current: WorkspaceSnapshot, id: string, code: number | null): WorkspaceSnapshot {
  const next = structuredClone(current)
  ...
  if (!found.window.layout.root) {
    ...
    if (live) {
      const used = new Set(next.spaces.flatMap((space) => space.windows.flatMap((window) => layoutPanes(window.layout.root).map((pane) => pane.id))))
...
function findWindow(workspace: WorkspaceSnapshot, target: { space?: string; window?: number }): { space: WorkspaceSpace; window: WorkspaceWindow } | null {
...
function findAgent(workspace: WorkspaceSnapshot, id?: string): { space: WorkspaceSpace; window: WorkspaceWindow; agent: PersistedAgent } | null {
  if (!id) {
    const target = findWindow(workspace, {})
    const pane = layoutPanes(target?.window.layout.root ?? null).find((item) => item.id === target?.window.state.focus)
...
function normalizeWindowState(window: WorkspaceWindow): void {
  ...
  if (window.state.zoom && (!panes.has(window.state.zoom.pane) ||
    !layoutPanes(window.state.zoom.from.root).some((pane) => pane.id === window.state.zoom!.pane))) {
    window.state.zoom = null
  }
}

function removeWindow(workspace: WorkspaceSnapshot, space: WorkspaceSpace, window: WorkspaceWindow, actions: WorkspaceAction[]): void {
  const at = space.windows.indexOf(window)
  if (at === -1) return
  for (const agent of window.agents) if (!agent.exited) actions.push({ _tag: "kill", agent: agent.id })
  space.windows.splice(at, 1)
  space.state = closeWindowState(space.state, space.windows.map((item) => item.number), window.number, at)
  if (space.windows.length === 0) removeSpace(workspace, space, actions)
}

function removeSpace(workspace: WorkspaceSnapshot, space: WorkspaceSpace, actions: WorkspaceAction[]): void {
  ...
  workspace.state = removeSpaceState(workspace.state, workspace.spaces.map((item) => item.id), space.id, at)
}
