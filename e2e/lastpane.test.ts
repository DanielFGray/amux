...
test(
  "^a c makes the new window active",
  async () => {
...
test(
  "^a l returns to the previous window",
  async () => {
...
test(
  "^a l toggles back to the window it left",
  async () => {
    await app.press(`${LEADER}l`);
...
test(
  "^a ; returns to the pane a split came from",
  async () => {
    await app.press(`${LEADER};`);
    await app.until(async () => {
      const now = await focusedPane(2);
...
test(
  "^a ; toggles back to the split's new pane",
  async () => {
    await app.press(`${LEADER};`);
