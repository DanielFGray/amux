...
beforeAll(async () => {
  ...
  await app.until(() => teeColumn(app.screen()) === even - 3, "the divider to move three cells left")
...
test("^a z fills the window, leaving no divider on screen", async () => {
  await app.press(`${LEADER}z`)
  await app.until(() => teeColumn(app.screen()) === -1, "the zoom to take the divider off screen")
  expect(teeColumn(app.screen())).toBe(-1)
}, E2E_TIMEOUT)

test("^a z again brings the second pane back", async () => {
  await app.press(`${LEADER}z`)
